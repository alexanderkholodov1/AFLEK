# AFLEK — implementation plan (v0.1 → v1.0)

> **Historical record.** FWP-01…09 are complete (see `docs/STATUS.md`); this document is kept
> as the build history and for the v0.2+ candidates in §5. "The source project" refers to the
> kit's first adopter, from which v0.1 was extracted.

> **Audience:** the maintainer and the agent sessions that build this repository.
> Self-contained: requires only this repository and read access to the source project. The charter
> (mission, doctrine, non-goals) is [`../README.md`](../README.md); this document is the **how**.
> Seed artifacts already in place: the charter, this plan, `templates/work-package.md`,
> `templates/AGENTS.template.md`, `playbooks/run-a-wave.md`, `playbooks/bootstrap-new-project.md`.
...
(PRs gated by CI). The source project is the proof: the kit is extracted from what already works there,
and that project becomes its first consumer.
...
| FD2 | **Cloud-first execution** | Local fan-out degraded the operator's machine and hid feedback (source project, June 2026). Local CLIs = one interactive session max. |
| FD3 | **One canonical `AGENTS.md`, shims point to it** | Multi-file drift is how the source project's entry points went stale; the doc-matrix rule makes shims update in the same PR. |
...
| FD5 | **Two-layer contract: kit structure + project domain** | Templates carry `{{PLACEHOLDERS}}` and a mandatory "project-specific guardrails" section; domain rules (e.g. the source project's scientific honesty) never live in the kit. |
...
| FD7 | **English, MIT, public** | Same reasoning as the source project's decision log. Third-party-derived files (ECC personas) keep attribution headers. |
...
└─ playbooks/
   ...
   ├─ adopt-existing-repo.md      retrofit (the source project is the worked example)
   ├─ run-a-wave.md               the orchestrator loop                    ← in this folder
   ├─ phase-audit.md              completeness audit at phase close (source-project audit as model)
...
Every FWP: one PR, acceptance criteria mechanically checkable, source project read-only as source.

| FWP | Deliverable | Source → transformation | Acceptance |
|---|---|---|---|
| 01 | Repo bootstrap: README, LICENSE, FLEET-VERSION, `doctrine/DOCTRINE.md` | this folder's README verbatim + expand each doctrine rule with "failure it prevents" (use the source-project audit as the casebook) | files exist; README quickstart ≤ 1 screen; CI badge optional |
| 02 | `templates/AGENTS.template.md` + 4 shims | `fleet/templates/AGENTS.template.md` (already drafted) + the source project's `GEMINI.md`, `.cursor/rules/00-agents.mdc`, `.github/copilot-instructions.md`, `CLAUDE.md` with project facts → `{{PLACEHOLDERS}}` | placeholders documented at top; no source-project nouns left (project-noun grep clean except in examples marked as examples) |
| 03 | `templates/CONTRIBUTING.template.md`, `changelog.d-README.md`, `STATUS.template.md`, `spec.template.md` | the source project's `CONTRIBUTING.md` (keep the PR-style do/don't table verbatim — it is the crown jewel), `changelog.d/README.md`, `docs/STATUS.md` structure, `planning/03` spec template | same de-projectification check |
| 04 | `templates/github/*` | the source project's `.github/workflows/ci.yml` (factor stack-specific steps behind a clearly-marked block), PR template, CODEOWNERS | template CI parses (`yamllint`); placeholder block documented |
...
| 07 | `playbooks/*` (6 files) | `run-a-wave.md` + `bootstrap-new-project.md` already drafted here; write the other four; `adopt-existing-repo.md` uses the source project as the worked example; `phase-audit.md` generalizes the June 2026 source-project audit method; `incident.md` includes the lost-write-access scenario (2026-06-12: keep committing locally, export patches to the operator) | every playbook is a numbered procedure with an explicit "done when" |
| 08 | **Source-project adoption pass** (in the adopter, not the kit) | replace nothing wholesale; add `FLEET-VERSION`, reconcile any divergence between adopter files and kit templates (expected: none — the kit was extracted from it), delete the incubation folder from the adopter | adopter CI green; incubation folder gone; STATUS notes adoption |
...
1. Session must have write scope on the kit repository and read on the source project.
...
4. On completion, run FWP-08 in the source project via a normal branch+PR.
